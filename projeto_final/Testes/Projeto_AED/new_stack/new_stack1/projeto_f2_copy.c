...
int maze_solver(FILE *fp_out, int *maze, int l, int c, int cells_number, int walls, int *walls_array, int a, int b)
{
    ...
    /* if (maze[conversion(a, b - 1, l, c)] == -1 && maze[conversion(a, b + 1, l, c)] == -1 && maze[conversion(a+1, b , l, c)] == -1 &&)
    {
        fprintf(fp_out,"%d\n",0);
        return 0;
    } */

    rooms = search_rooms(maze, l, c, cells_number, walls, walls_array);
    //printf("rooms=%d\n", rooms);
    ...
    //se a casa de chegada ou de partida tiverem numa sala blindada a -1
    if(maze[conversion(a,b,l,c)] == 0 || maze[conversion(1,1,l,c)] == 0){
        fprintf(fp_out,"%d\n",-1);
        return -1;
    }
    ...
    printf("%d-%d-%d\n",abs(maze[conversion(1,1,l,c)])-2,abs(maze[conversion(a,b,l,c)])-2,rooms); */

    int **result,*cost,*parent;
    result = dijkstra(adjList,maze,abs(maze[conversion(1,1,l,c)])-2,rooms);
    //return 0;
    ...
    for(int j=final_vertix;j!=-1;j=parent[j])   
        k++;   
    fprintf(fp_out,"%d\n",--k);  */      
    printPath(fp_out,maze,l,c,adjList,parent,final_vertix,1);
...
int search_rooms(int *maze, int l, int c, int cells_number, int walls, int *walls_array)
{
    int i, a1, b1, k = 0, *coord;
    Stack *white_stack;
    white_stack=NULL;
    ...
    for (i = 0; i < walls; i++)
    {
        ...
        {
            white_stack=push(white_stack, a1, b1 - 1);
            white_stack=walk_room(maze, white_stack, k, l, c);
            k++;
        }
        if (maze[conversion(a1 - 1, b1, l, c)] == 0)
        {
            white_stack=push(white_stack, a1 - 1, b1);
            white_stack=walk_room(maze, white_stack, k, l, c);
            k++;
        }
        if (maze[conversion(a1, b1 + 1, l, c)] == 0)
...
Stack * walk_room(int *maze,Stack *stack,int k, int l, int c)
{
    Stack *aux;
    int a, b;
    do
    {
        ...
        {
            stack=push(stack, a, b - 1);
        }
        if (maze[conversion(a - 1, b, l, c)] == 0)
        {
            stack=push(stack, a - 1, b);
        }
        if (maze[conversion(a, b + 1, l, c)] == 0)
        {
            stack=push(stack, a, b + 1);
...
void printPath(FILE *fp_out, int *maze,int l,int c,LinkedList** adjList,int* parent, int j,int k)
{
    ...
    k++;   
    printPath(fp_out,maze,l,c,adjList,parent, parent[j],k);
    
    LinkedList* Node = get_vertix(adjList[j],parent[j],INT_MAX);
    
    int *coord = inverted_conversion(Node->Weight,l,c);
    
    fprintf(fp_out, "%d %d %d\n",coord[0],coord[1], maze[Node->Weight]);
    free(coord);
}

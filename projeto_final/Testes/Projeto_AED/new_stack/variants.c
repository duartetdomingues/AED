...
int a6(int a, int b, int a1, int b1, int l, int c, int cells_number, int *maze){
    ...
    white_stack=push(white_stack,a,b);
    ...
    do{
        a2 = pop_a(white_stack);
        b2 = pop_b(white_stack);
        aux=white_stack;
        white_stack=white_stack->next;
        free(aux);

        /* Testing the possible paths in the 4 directions that allow you to proceed a valid path */
        if (maze[conversion(a2, b2 - 1, l, c)] == 0 && check[conversion(a2, b2 - 1, l, c)] != 1){
            check[conversion(a2, b2 -1, l, c)] = 2;
            white_stack=push(white_stack, a2, b2 - 1);
        }
        if (maze[conversion(a2, b2 + 1, l, c)] == 0 && check[conversion(a2, b2 + 1, l, c)] != 1){
            check[conversion(a2, b2 + 1, l, c)] = 2;
            white_stack=push(white_stack, a2, b2 + 1);;
        }
        if (maze[conversion(a2 - 1, b2, l, c)] == 0 && check[conversion(a2 - 1, b2, l, c)] != 1){
            check[conversion(a2 -1, b2, l, c)] = 2;
            white_stack=push(white_stack, a2 - 1, b2);
        }
        if (maze[conversion(a2 + 1, b2, l, c)] == 0 && check[conversion(a2 + 1, b2, l, c)] != 1){
            check[conversion(a2 + 1, b2, l, c)] = 2;
            white_stack=push(white_stack, a2 + 1, b2);
    ...
    } while (check[conversion(a1, b1, l, c)] == 0 && isEmpty(white_stack) != 1);

    freeStack(white_stack);
    
    /* Freeing the data structure */
    if (check[conversion(a1, b1, l, c)] == 0){
        free(check);
        return 0;
    }
    else{

...
int main(int argc, char *argv[]){
    ...
    while (!feof(fp_in) && fscanf(fp_in, "%d %d", &l, &c) == 2){
        ...
        int *walls_array = (int *)calloc(walls+1, sizeof(int));
        ...
        for (i = 0; i < walls; i++){
            ...
            walls_array[i+1]=conversion(x, y, l, c);
        }

        //casas cinzentas impossivei
        int j=0;
        for (int i = 0; i <= walls; i++,j++){
                int* k;
                k= inverted_conversion(walls_array[i],l,c);
                walls_array[j]=walls_array[i];
                if (variants(5, maze, l, c, cells_number, k[0], k[1], 0, 0) == 0 || maze[walls_array[i]]==-1 )
                {
                j--;

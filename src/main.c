 ...
 *
 * DESCRIPTION
 *      Main program to read the given file and call the maze_solver function
 *      which will solve the given maze using different types of algorithms like
 *      BFS and Dijkstras.
 *
 ******************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2 || strstr(argv[1], ".in") == NULL)
    {
        exit(EXIT_FAILURE);
    }
    ...
    while (!feof(fp_in) && fscanf(fp_in, "%d %d", &l, &c) == 2)
    {
        ...
        int *walls_array = (int *)calloc(walls, sizeof(int));
        ...
        for (i = 0; i < walls; i++)
        {
            ...
            walls_array[i] = conversion(x, y, l, c);
        }
        /* Filling an array with the 'breakable' grey cells */
        int j = 0;
        for (int i = 0; i < walls; i++, j++)
        {
            int *k;
            k = inverted_conversion(walls_array[i], l, c);
            walls_array[j] = walls_array[i];
            if (a5( maze, l, c, k[0], k[1]) == 0 || maze[walls_array[i]] == -1)
            {
                j--;
                maze[conversion(k[0], k[1], l, c)] = -1;
            }
        ...
        maze_solver(fp_out, maze, l, c, cells_number, walls, walls_array, a, b);
 ...
 * Arguments: x, l, c
 *
 * Returns: a pair of coordinates, given the number of the cell
 *
 * Description: converts a 1D coordinate into a 2D coordinate (to the
 ...
 * a5()
 *
 * Arguments: maze, l, c, a, b
 * Returns: -1, 0 or 1.
 *
 * Description: This function tests the 'breakability' of a grey cell. In
 * fact, this function returns -1 if the cell isn't grey at all. If it is,
 * it eithers return 0 if it isn't breakable or 1 if it is.
 * 
 *****************************************************************************/
...
void print_maze(int* maze, int l, int c){
    int i,j, k=0;
    for(i=0; i<c; i++){
        for(j=0; j<l;j++){
            k++;

...
int maze_solver(FILE *fp_out, int *maze, int l, int c, int cells_number, int walls, int *walls_array, int a, int b)
{
    ...
    rooms = makeRooms(maze, l, c, cells_number);
 ...
 *  
 *****************************************************************************/
int makeRooms(int *maze, int l, int c, int cells_number)
{
    Queue *roomStack = NULL, *stackTail = NULL;
    int i, j, k = 0, a, b, a1, b1, *coord;
    int rowNum[] = {-1, 0, 0, 1};
    int colNum[] = {0, -1, 1, 0};

    /* Transversing the whole array */
    for (i = 1; i <= cells_number; i++)
    {   
        //if it's a valid source point apply BFS to it
        if(maze[i]==0){
            maze[i] = (-k - 2);
            push(&roomStack, &stackTail, i);

            while (isEmpty(roomStack) != 1)
            {
                //obtaining 2D coordinates of a cell
                coord = inverted_conversion(roomStack->vertex, l, c);
                a = coord[0];
                b = coord[1];
                free(coord);

                roomStack = pop(&roomStack);
                for (j = 0; j < 4; j++)
                {
                    a1 = a + rowNum[j];
                    b1 = b + colNum[j];
